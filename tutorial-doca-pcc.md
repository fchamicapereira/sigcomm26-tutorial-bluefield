<!--
  ...
  hand-hold every command, walk through code rather than saying "go read it," INFO/NOTE callouts,
  source line-links, collapsible "Try it yourself!" blocks, Debugging Tips, FAQs.

  DESIGN (matches Part I): PCC now HAS a fill-in exercise. device/algo/rtt_template.c is the
  scaffold shipped to participants — a copy of the finished controller with the two rate reactions
  (CNP multiplicative decrease = TODO 1, TX additive increase = TODO 2) hollowed out. Participants
  fill the two TODOs in Part C; the finished controller is the solution (the -solutions repo, and
