# `doca_flow_template` — build the ECN-marking pipeline yourself
...
### TODO 2 — `bind_capture_mirror`
What feeds copies to that pipe. A shared mirror carries a list of targets; here there is exactly
one. Give `target` a `DOCA_FLOW_FWD_PIPE` forward whose `next_pipe` is `cpu_pipe`, attach it to `mc`
(`nr_targets`, `target`), then `doca_flow_shared_resource_set_cfg` and
`doca_flow_shared_resources_bind` it under `MIRROR_ID`.

Note the two halves: `target.fwd` is where the **copy** goes, while `DOCA_TUT_MIRROR_SET_ORIG_FWD`
decides where the **original** continues. Getting the copy wrong loses the pcap; getting the
original wrong black-holes the data path.
...
- When `mirror` is true, set `mon.shared_mirror_id` to `MIRROR_ID`. Leaving it `0` means "no
  mirror", which is exactly right for the non-capturing case.
...
- `mirrored:` climbs, showing copies reaching the CPU queue.
