...
static doca_error_t open_pcc_device(const char *device_name, struct doca_dev **doca_device)
{
	...
	*doca_device = NULL;
	for (uint32_t i = 0; i < nb_devs; i++) {
		if (doca_devinfo_get_is_pcc_supported(dev_list[i]) != DOCA_SUCCESS)
			continue;
...
int main(int argc, char **argv)
{
	...
	if (result == DOCA_SUCCESS)
		result = doca_pcc_get_max_num_threads(pcc, &max_threads);
	...
	if (result == DOCA_SUCCESS)
		result = doca_pcc_rp_set_ccmad_remote_sw_handler(pcc, 0, false);
	if (result != DOCA_SUCCESS) {
	...
	if (result == DOCA_SUCCESS)
		result = doca_pcc_set_trace_message(pcc, trace_message_formats);
	if (result == DOCA_SUCCESS)
		result = doca_pcc_set_dev_coredump_file(pcc, PCC_COREDUMP_FILE_PATH);
	if (result != DOCA_SUCCESS) {
