...
static struct doca_flow_pipe *create_sampling_pipe(struct doca_flow_port *port,
  ...
  fwd_hit.next_pipe = hit;
  fwd_miss.type = DOCA_FLOW_FWD_PIPE;
  fwd_miss.next_pipe = miss;
...
static void build_pipeline(struct doca_flow_port *port, const struct app_config *cfg,
  ...
  // PASS forwards and counts; MARK also rewrites the ECN bits to CE. Anything they do not match
  // misses into PASSTHROUGH and reaches the SF anyway, so the match only decides what is counted
  // and marked, never what gets through.
