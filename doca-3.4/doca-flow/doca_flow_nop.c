 ...
 *
 * Same environment as doca_flow_mac / doca_flow_ecn — see setup/setup_roce_loopback.sh / README.md.
 */
#include <doca_argp.h>
#include <doca_dev.h>
#include <doca_dpdk.h>
...
static struct doca_flow_pipe_entry *add_fwd_entry(struct doca_flow_pipe *pipe, struct doca_flow_port *port, bool with_counter) {
  ...
  err = doca_flow_pipe_basic_add_entry(0, pipe, &match, 0, NULL, with_counter ? &monitor : NULL, NULL, DOCA_FLOW_ENTRY_FLAGS_NO_WAIT,
                                       &status, &entry);
...
static struct doca_flow_pipe *create_port_demux_pipe(struct doca_flow_port *port, struct doca_flow_pipe *fwd_pipe) {
  ...
  err = doca_flow_pipe_basic_add_entry(0, pipe, &entry_match, 0, NULL, NULL, &entry_fwd, DOCA_FLOW_ENTRY_FLAGS_WAIT_FOR_BATCH, &status,
                                       &entry);
  ...
  entry_fwd.port_id = 0;
  err = doca_flow_pipe_basic_add_entry(0, pipe, &entry_match, 0, NULL, NULL, &entry_fwd, DOCA_FLOW_ENTRY_FLAGS_NO_WAIT, &status, &entry);
