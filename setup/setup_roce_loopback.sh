...
echo "== ensuring OVS bridges exist (ovsbr1: PF0 side, ovsbr2: PF1 side) =="
# This is the "kernel default" forwarding path referenced throughout the README: an
# ASAP²-hardware-offloaded L2 learning bridge per PF. DOCA Flow only ever takes over PF0's FDB
# (fdb_def_rule_en=1 keeps PF1 on this path), so PF1's traffic — and PF0's, whenever no
# doca-flow program is running — depends on these bridges existing. Idempotent: safe to re-run.
sudo ovs-vsctl --may-exist add-br ovsbr1
sudo ovs-vsctl --may-exist add-port ovsbr1 p0
sudo ovs-vsctl --may-exist add-port ovsbr1 pf0hpf
sudo ovs-vsctl --may-exist add-port ovsbr1 en3f0pf0sf0

sudo ovs-vsctl --may-exist add-br ovsbr2
sudo ovs-vsctl --may-exist add-port ovsbr2 p1
sudo ovs-vsctl --may-exist add-port ovsbr2 pf1hpf
sudo ovs-vsctl --may-exist add-port ovsbr2 en3f1pf1sf0
...
echo "Next: build (see README), start a doca-flow program on PF0, then doca_pcc on PF1, then drive traffic."
