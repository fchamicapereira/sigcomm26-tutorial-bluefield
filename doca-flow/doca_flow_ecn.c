...
/* Marking mode, selected on the command line (see register_ecn_params). */
enum ecn_mode {
  ECN_MODE_RANDOM, /* mode 1: mark a uniform-random fraction of packets, 0-100% (see --percent) */
  ECN_MODE_TOGGLE, /* mode 2: state machine toggled by pressing <Enter> */
};

struct ecn_app_config {
  enum ecn_mode mode;
  double random_percent; /* mode 1 only: requested percentage of packets to mark, [0, 100] */
};
 ...
 *
 * Used as the miss target (INGRESS_PASSTHROUGH) for the ECN mark pipe: non-IPv4 and (in mode 1,
 * random) not-selected wire packets reach mlx5_2 via port 1 as-is. No rewrite is needed to get there:
...
/*
 * Mode 1 (random), 0 < percent < 100: a dedicated pre-filter pipe that matches ONLY
 * parser_meta.random (no header fields, no actions) and forwards the selected fraction to
 * hit_pipe (ECN_MARK) or everything else to miss_pipe (INGRESS_PASSTHROUGH). Mirrors DOCA's
 * own flow_random sample (doca_flow/flow_random/flow_random_sample.c) exactly.
 *
 ...
 * (mode 1) or the passthrough pipe (mode 2, starting in NOT_MARK; also mode 1 at --percent 0).
 * wire_entry_out returns the port_meta==0 entry so mode 2 can retarget it live via
 * doca_flow_pipe_update_entry.
 */
static struct doca_flow_pipe *create_port_demux_pipe(struct doca_flow_port *port, struct doca_flow_pipe *wire_ingress_target,
                                                     struct doca_flow_pipe_entry **wire_entry_out) {
  struct doca_flow_match match = {0};
  struct doca_flow_match match_mask = {0};
...
/*
 * Mode 2 (toggle): flips the PORT_DEMUX wire-ingress entry's forward target between the
 * ECN_MARK pipe (MARK) and the passthrough pipe (NOT_MARK), live, via
 * doca_flow_pipe_update_entry — no new entries, no pipe rebuild.
 */
static void toggle_mark_state(bool *marking, struct doca_flow_port *port, struct doca_flow_pipe *demux_pipe,
                              struct doca_flow_pipe_entry *wire_entry, struct doca_flow_pipe *ecn_pipe,
                              struct doca_flow_pipe *passthrough_pipe) {
  *marking = !*marking;

  struct doca_flow_fwd fwd = {.type = DOCA_FLOW_FWD_PIPE, .next_pipe = *marking ? ecn_pipe : passthrough_pipe};
  doca_error_t err = doca_flow_pipe_update_entry(0, demux_pipe, NULL, NULL, &fwd, 0, wire_entry);
  crash_if_unsuccessful(err, "doca_flow_pipe_update_entry (toggle)");

  err = doca_flow_entries_process(port, 0, 10000, 1);
  crash_if_unsuccessful(err, "doca_flow_entries_process (toggle)");

  DOCA_LOG_INFO("Mode 2 (toggle): now %s", *marking ? "MARK" : "NOT_MARK");
}
...
static doca_error_t mode_callback(void *param, void *config) {
  ...
  if (strcmp(mode_str, "random") == 0) {
    cfg->mode = ECN_MODE_RANDOM;
  } else if (strcmp(mode_str, "toggle") == 0) {
    cfg->mode = ECN_MODE_TOGGLE;
  } else {
    DOCA_LOG_ERR("Invalid --mode '%s' (expected: random, toggle)", mode_str);
    return DOCA_ERROR_INVALID_VALUE;
  }
  return DOCA_SUCCESS;
}
...
static void register_ecn_params(void) {
  struct doca_argp_param *mode_param, *percent_param;
  doca_error_t err;

  err = doca_argp_param_create(&mode_param);
  crash_if_unsuccessful(err, "doca_argp_param_create (mode)");
  doca_argp_param_set_short_name(mode_param, "m");
  doca_argp_param_set_long_name(mode_param, "mode");
  doca_argp_param_set_description(mode_param,
                                  "Marking mode: 'random' (mark a uniform-random fraction, see --percent; 100 marks "
                                  "everything, 0 marks nothing), 'toggle' (start NOT_MARK, flip MARK/NOT_MARK on <Enter>). "
                                  "Default: random");
  doca_argp_param_set_callback(mode_param, mode_callback);
  doca_argp_param_set_type(mode_param, DOCA_ARGP_TYPE_STRING);
  err = doca_argp_register_param(mode_param);
  crash_if_unsuccessful(err, "doca_argp_register_param (mode)");
  ...
  doca_argp_param_set_description(percent_param,
                                  "Mode 'random' only: requested percentage of packets to mark, [0, 100] (fractional values "
                                  "allowed, e.g. 0.1). 0 and 100 are exact; anything in between is rounded down to the "
                                  "nearest power-of-two fraction: 50, 25, 12.5, .... Default: 100");
...
int main(int argc, char **argv) {
  ...
  struct ecn_app_config app_cfg = {.mode = ECN_MODE_RANDOM, .random_percent = 100.0};
  ...
  /* INGRESS_PASSTHROUGH: miss target of ECN_MARK/RANDOM_SAMPLE, mode-2's NOT_MARK state, and
   * --percent 0's only route — non-ECT / already-CE / not-selected / never-selected wire packets
   * still reach mlx5_2 (port 1) untouched. Counted, so every mode can be observed live. */
  ...
  /* ECN_MARK only needs to exist if something can actually reach it: mode toggle (always, since
   * it can flip to MARK live) or mode random with a nonzero percentage. At --percent 0 nothing
   * ever marks, so there's no pipe to build (entries.ce stays NULL — see print_stats). */
  bool need_ecn_mark_pipe = (app_cfg.mode == ECN_MODE_TOGGLE) || (app_cfg.mode == ECN_MODE_RANDOM && app_cfg.random_percent > 0.0);
  struct doca_flow_pipe *pipe = NULL;
  struct ecn_entries entries = {0};
  if (need_ecn_mark_pipe) {
    pipe = create_ecn_mark_pipe(port, ingress_pass);
    entries = add_ecn_entries(pipe, port);
  }
  entries.passthrough = pass_entry;

  /* Mode 1 (random): RANDOM_SAMPLE pre-filters p0 wire ingress by a HW random value before
   * ECN_MARK ever sees it. Only built for 0 < percent < 100 — at 100% everything goes straight
   * to ECN_MARK (no need to sample first), at 0% everything goes straight to ingress_pass. */
  uint16_t random_mask = 0;
  struct doca_flow_pipe *sample_pipe = NULL;
  struct doca_flow_pipe *wire_ingress_target;
  if (app_cfg.mode == ECN_MODE_RANDOM) {
    if (app_cfg.random_percent >= 100.0) {
      ...
      sample_pipe = create_random_sample_pipe(port, pipe, ingress_pass, random_mask);
      wire_ingress_target = sample_pipe;
    }
  } else {
    wire_ingress_target = ingress_pass; /* mode toggle starts NOT_MARK */
  }
  ...
  bool marking = false; /* only meaningful for mode toggle, which always starts NOT_MARK */
  struct doca_flow_pipe_entry *wire_entry;
  struct doca_flow_pipe *demux_pipe = create_port_demux_pipe(port, wire_ingress_target, &wire_entry);

  signal(SIGINT, signal_handler);
  signal(SIGTERM, signal_handler);

  switch (app_cfg.mode) {
    case ECN_MODE_RANDOM:
      if (app_cfg.random_percent >= 100.0) {
        DOCA_LOG_INFO("Mode 1 (random): marking every IPv4 packet — Ctrl-C to stop");
      } else if (app_cfg.random_percent <= 0.0) {
        DOCA_LOG_INFO("Mode 1 (random): marking no packets — Ctrl-C to stop");
      } else {
        DOCA_LOG_INFO("Mode 1 (random): marking ~%.4g%% of IPv4 packets — Ctrl-C to stop", 100.0 / (random_mask + 1));
      }
      break;
    case ECN_MODE_TOGGLE:
      DOCA_LOG_INFO("Mode 2 (toggle): starting NOT_MARK — press <Enter> to flip MARK/NOT_MARK, Ctrl-C to stop");
      break;
  }

  while (g_running) {
    if (app_cfg.mode == ECN_MODE_TOGGLE) {
      /* Poll stdin so a keypress reacts immediately, at the same ~1s cadence as the other
       * modes' sleep(1) when idle. */
      struct pollfd pfd = {.fd = STDIN_FILENO, .events = POLLIN};
      int ready = poll(&pfd, 1, 1000 /* ms */);
      if (ready > 0 && (pfd.revents & POLLIN)) {
        char buf[64];
        (void)read(STDIN_FILENO, buf, sizeof(buf)); /* drain; any input toggles */
        toggle_mark_state(&marking, port, demux_pipe, wire_entry, pipe, ingress_pass);
      }
    } else {
      sleep(1);
    }
