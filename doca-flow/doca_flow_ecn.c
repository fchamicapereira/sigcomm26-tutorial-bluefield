...
static void probe_device(struct ecn_app_config *cfg) {
  doca_error_t err;
...
static void configure_and_start_dpdk_port(struct doca_dev *dev) {
  ...
  DOCA_LOG_INFO("First DPDK port ID for device: %u, total available: %u", first_port_id, rte_eth_dev_count_avail());
...
static struct doca_flow_port *port_start(struct doca_dev *dev, uint16_t flow_port_id) {
  DOCA_LOG_INFO("Starting DOCA Flow port with flow_port_id=%u", flow_port_id);
...
static struct doca_flow_port *rep_port_start(uint16_t flow_port_id, struct doca_dev_rep *dev_rep) {
  DOCA_LOG_INFO("Starting rep port with flow_port_id=%u, dev_rep=%p", flow_port_id, (void *)dev_rep);
  struct doca_flow_port_cfg *cfg;
 ...
 */
static struct doca_flow_pipe *create_ecn_mark_pipe(struct doca_flow_port *port, struct doca_flow_pipe *miss_pipe, uint16_t dest_port_id) {
  struct doca_flow_match match = {0};
  struct doca_flow_match match_mask = {0};
  ...
  /* CE mark → deliver to SF rep (the ibverbs receiver) */
...
static void create_port_demux_pipe(struct doca_flow_port *port, struct doca_flow_pipe *wire_ingress_target) {
  ...
  doca_flow_pipe_cfg_destroy(cfg);
  DOCA_LOG_INFO("PORT_DEMUX pipe created, adding entries...");
...
int main(int argc, char **argv) {
  ...
  struct ecn_app_config app_cfg = {.random_percent = 100.0, .dev = NULL, .dev_rep = NULL, .devargs = NULL};
  ...
  /* Use logical port IDs: 0 = PF uplink, 1 = SF representor */
