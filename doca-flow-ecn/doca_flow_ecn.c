...
enum ecn_mode {
  ECN_MODE_ALL,    /* mode 1: mark every IPv4 packet unconditionally */
  ECN_MODE_RANDOM, /* mode 2: mark a uniform-random fraction of packets */
  ECN_MODE_TOGGLE, /* mode 3: state machine toggled by pressing <Enter> */
};

struct ecn_app_config {
  enum ecn_mode mode;
  int random_percent; /* mode 2 only: requested percentage of packets to mark, 0-100 */
};
...
static uint16_t get_random_mask(double percentage) {
  ...
  if (percentage > next_supported) {
    DOCA_LOG_WARN("Requested %.4g%% sampling not supported (power-of-2 only); using %.4g%% instead", percentage,
                  next_supported);
  }
                                                  ...
                                                  bool with_counter)
{
  ...
  err = doca_flow_pipe_add_entry(0, pipe, &match, &actions, with_counter ? &monitor : NULL, NULL, 0, &status, &entry);
 ...
 *
 * Non-IPv4 packets miss this pipe and go to miss_pipe (forward-only, no rewrite). In mode 2
 * (random), only the packets selected by RANDOM_SAMPLE (see create_random_sample_pipe) ever
 * reach this pipe at all — everything else is diverted to miss_pipe upstream, before here.
 */
...
/*
 * Mode 2 (random): a dedicated pre-filter pipe that matches ONLY parser_meta.random (no
 ...
 */
static struct doca_flow_pipe *create_random_sample_pipe(struct doca_flow_port *port,
 ...
 *
 * wire_ingress_target selects where p0-wire ingress starts: the ECN_MARK pipe (mode 1/2)
 * or the passthrough pipe (mode 3, starting in NOT_MARK). wire_entry_out returns the
 * port_meta==0 entry so mode 3 can retarget it live via doca_flow_pipe_update_entry.
 */
...
static void print_stats(struct ecn_entries *entries) {
  ...
  uint64_t ce_pkts = ce_query.counter.total_pkts;
  ...
  DOCA_LOG_INFO("CE marked: %lu, passthrough: %lu (%.2f%% marked)", ce_pkts, pass_pkts, pct);
}

/*
 * Mode 3 (toggle): flips the PORT_DEMUX wire-ingress entry's forward target between the
...
static void toggle_mark_state(bool *marking, struct doca_flow_port *port, struct doca_flow_pipe *demux_pipe,
  ...
  *marking = !*marking;
  ...
  DOCA_LOG_INFO("Mode 3: now %s", *marking ? "MARK" : "NOT_MARK");
}
...
static doca_error_t percent_callback(void *param, void *config) {
  ...
  if (percent < 1 || percent > 100) {
    DOCA_LOG_ERR("--percent must be between 1 and 100 (got %d)", percent);
    return DOCA_ERROR_INVALID_VALUE;
  }
...
static void register_ecn_params(void) {
  ...
  doca_argp_param_set_description(mode_param,
      "Marking mode: 'all' (mark every IPv4 packet), 'random' (mark a uniform-random fraction, "
      "see --percent), 'toggle' (start NOT_MARK, flip MARK/NOT_MARK on <Enter>). Default: all");
  ...
  doca_argp_param_set_description(percent_param,
      "Mode 'random' only: requested percentage of packets to mark (1-100, rounded down to the "
      "nearest power-of-two fraction: 50, 25, 12.5, ...). Default: 50");
  doca_argp_param_set_callback(percent_param, percent_callback);
  doca_argp_param_set_type(percent_param, DOCA_ARGP_TYPE_INT);
...
int main(int argc, char **argv) {
  ...
  struct ecn_app_config app_cfg = {.mode = ECN_MODE_ALL, .random_percent = 50};
  ...
  /* INGRESS_PASSTHROUGH: miss target of ECN_MARK, and mode-3's NOT_MARK state — non-ECT /
   * already-CE / not-selected wire packets still reach mlx5_2 (port 1), with dst MAC
   * rewritten so the ibverbs QP rule fires. Counted, so mode 2/3 can be observed live. */
  ...
  struct doca_flow_pipe_entry *pass_entry =
      add_fwd_entry(ingress_pass, port, "ingress passthrough", RECEIVER_MAC, /*with_counter=*/true);

  /* ECN_MARK (non-root): marks all IPv4 reaching it → CE, forwards to mlx5_2; misses (non-IPv4)
   * go to ingress_pass. In mode 2, only the RANDOM_SAMPLE-selected fraction ever reaches it. */
  ...
  /* Mode 2 (random): RANDOM_SAMPLE pre-filters p0 wire ingress by a HW random value before
   * ECN_MARK ever sees it — selected fraction -> ECN_MARK, the rest -> ingress_pass. */
  uint16_t random_mask = (app_cfg.mode == ECN_MODE_RANDOM) ? get_random_mask(app_cfg.random_percent) : 0;
  struct doca_flow_pipe *sample_pipe =
      (app_cfg.mode == ECN_MODE_RANDOM) ? create_random_sample_pipe(port, pipe, ingress_pass, random_mask) : NULL;

  /* PORT_DEMUX (root): splits by source port. p0 wire ingress -> ECN_MARK (mode 1), RANDOM_SAMPLE
   * (mode 2), or ingress_pass (mode 3, which starts in NOT_MARK); mlx5_2 SF egress (ACKs/CNPs) ->
   * p0 wire so they return to mlx5_3. Replaces the old EGRESS_FORWARD pipe, which never fired
   * because the DEFAULT-domain root pipe made the forwarding decision before egress processing. */
  bool marking = (app_cfg.mode == ECN_MODE_ALL); /* mode 2 has its own gate; mode 3 starts NOT_MARK */
  struct doca_flow_pipe *wire_ingress_target =
      (app_cfg.mode == ECN_MODE_RANDOM) ? sample_pipe : (marking ? pipe : ingress_pass);
  ...
  switch (app_cfg.mode) {
  case ECN_MODE_ALL:
    DOCA_LOG_INFO("Mode 1 (all): marking every IPv4 packet — Ctrl-C to stop");
    break;
  case ECN_MODE_RANDOM:
    DOCA_LOG_INFO("Mode 2 (random): marking ~%.4g%% of IPv4 packets — Ctrl-C to stop",
                  100.0 / (random_mask + 1));
    break;
  case ECN_MODE_TOGGLE:
    DOCA_LOG_INFO("Mode 3 (toggle): starting NOT_MARK — press <Enter> to flip MARK/NOT_MARK, Ctrl-C to stop");
    break;
  }
